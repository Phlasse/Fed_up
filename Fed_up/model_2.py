...
def __create_latent_matrices(pool = 2000, content_reduction = 250, rating_reduction = 800, user_inputs=None):
    ...
    recipes_df_raw = pd.read_csv("data/preprocessed/data_preprocessed_recipe_pp.csv")
    reviews_df_raw = pd.read_csv("data/preprocessed/data_preprocessed_review_pp.csv")
    ...
    ### filter method here:
    input_df = pd.concat([user_recipe_df, filtered_df.sample(pool)], axis=0)
...
def get_user_recommendations(user_inputs=None, n_recommendations=None, collaborative=0.5, clear_neg=True):
    ...
    content_latent, rating_latent = __create_latent_matrices(2000, 250, 800, user_inputs)
    ...
    if user_dislikes:
        dislikes = [get_one_recommendation(i, content_latent, rating_latent, collaborative)*-1 for i in user_dislikes if i != []]
