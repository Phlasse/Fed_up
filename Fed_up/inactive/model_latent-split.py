...
def create_latent_matrices(max_parts = 10000, content_reduction = 250, rating_reduction = 800):
    ...
    recipes_df_raw = pd.read_csv("data/preprocessed/recipe_pp.csv")
    reviews_df_raw = pd.read_csv("data/preprocessed/review_pp.csv")
...
def get_one_recommendation(recipe_id, content_latents, rating_latents, collaborative=0.5):
...
def get_user_recommendations(user_inputs=None, n_recommendations=None, collaborative=0.5, clear_neg=True):
    ...
    content_latents, rating_latents = __get_prediction_data()
    present_recipe_ids = []
    ...
    if user_likes:
        recommendations = [get_one_recommendation(i, content_latents, rating_latents, collaborative) for i in user_likes if i != []]
        recommendations_df = pd.concat(recommendations)

    ### add negative recommendations ###
    if user_dislikes:
        dislikes = [get_one_recommendation(i, content_latents, rating_latents, collaborative)*-1 for i in user_dislikes if i != []]
...
def __get_prediction_data():
    content_latents = []
    rating_latents = []
    latent_mats = 9

    csv_path = os.path.join(os.path.dirname(__file__), "data/latents")

    for i in range(latent_mats):
        latent_1 = pd.read_csv(f"{csv_path}/latent_content_{i}.csv").rename(columns={"Unnamed: 0":"recipe_id"})
        latent_2 = pd.read_csv(f"{csv_path}/latent_rating_{i}.csv").rename(columns={"Unnamed: 0":"recipe_id"})
